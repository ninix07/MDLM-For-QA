...
def check_kill_signals(logs: dict, step: int, epoch: int):
    ...
    if logs["vital/vae_recon_per_valid"] > 6.0 and epoch > 0:
        warnings.append(
            f"🚨 HIGH RECON LOSS: {logs['vital/vae_recon_per_valid']:.2f} > 6.0 (VAE not learning)"
        )
    ...
    if logs["vital/vae_active_latent_dims"] < 10:
        warnings.append(
            f"🚨 FEW ACTIVE DIMS: {logs['vital/vae_active_latent_dims']} < 10 (wasted capacity)"
        )
    ...
    if "vital/null_prediction_rate" in logs:
        if logs["vital/null_prediction_rate"] > 0.95:
            warnings.append(
                f"🚨 NULL BIAS: {logs['vital/null_prediction_rate']:.1%} > 95% saying 'no answer'"
            )
        elif logs["vital/null_prediction_rate"] < 0.05 and epoch > 0:
            warnings.append(
                f"🚨 IGNORING NULL: {logs['vital/null_prediction_rate']:.1%} < 5% unanswerable predictions"
            )
...
def debug_dimensions(model, batch, device, epoch_num):
    ...
    with torch.no_grad():
        ...
        print(
            f"    Sample Answer: '{answer_text[:50]}...' "
            if len(answer_text) > 50
            else f"    Sample Answer: '{answer_text}'"
        )
        print(f"    BERT Embeddings: {embeddings.shape} (expected: [1, seq, 768])")
        print(
            f"    VAE Latent (z):  {z.shape} (expected: [1, 8, {model.latent_dim}] for VAE or [1, seq, 768] for Bridge)"
        )
        print(
            f"    Transformation: {embeddings.shape[-1]} → {z.shape[-1]} ✓"
            if z.shape[-1] == model.latent_dim
            else f"    ❌ ERROR: Expected {model.latent_dim}, got {z.shape[-1]}"
        )
    model.train()
...
def validate(
    ...
    for batch_idx, batch in enumerate(tqdm(val_loader, desc="Validating")):
        ...
        if batch_idx % 10 == 0:
            ...
            if batch_idx % 50 == 0 and vae_output["z"] is not None:
                _, _, _, t_logs = log_token_accuracy(
                    model, batch, vae_output, global_step + batch_idx, log_to_wandb=False
                )
        ...
        if num_batches <= max_metric_batches:
            if train_vae_only:
                # During VAE warmup, test reconstruction quality
                gen_outputs = model.vae_reconstruct(answer_ids, answer_mask)

                # Also log a warning that these are reconstruction metrics, not generation
                if num_batches == 1:
                    print(
                        "[WARNING] VAE warmup metrics are reconstruction-only (not true generation)"
                    )
            else:
    ...
    if debug_sample:
        ...
        if "latent_shape" in debug_sample:
            ...
            print(
                f"Latent Mean: {debug_sample['latent_mean']:.4f}, Std: {debug_sample['latent_std']:.4f}"
            )
    ...
    if all_vital_logs:
        ...
        for key in all_vital_logs[0].keys():
            ...
            values = [
                log[key]
                for log in all_vital_logs
                if key in log and isinstance(log[key], (int, float))
            ]
            if values:
...
def main():
    ...
    parser.add_argument(
        "--lr", type=float, default=None, help="Learning rate (overrides config if set)"
    )
    ...
    if not skip_warmup and config.training.vae_warmup_epochs > 0 and args.use_vae:
        ...
        for epoch in range(config.training.vae_warmup_epochs):
            ...
            pbar = tqdm(
                train_loader, desc=f"Warmup Epoch {epoch+1}/{config.training.vae_warmup_epochs}"
            )
            ...
            print(
                f"Warmup Epoch {epoch+1}: Train VAE Loss = {avg_vae_loss:.4f}, Val VAE Loss = {val_vae_loss:.4f} (Recon={val_recon:.4f}, KL={val_kl:.4f}), F1 = {val_f1:.2f}, EM = {val_em:.2f}"
            )
            print(
                f"    HasAns: F1 = {val_metrics.get('has_ans_f1', 0):.2f}, EM = {val_metrics.get('has_ans_em', 0):.2f}"
            )
            print(
                f"    NoAns:  F1 = {val_metrics.get('no_ans_f1', 0):.2f}, EM = {val_metrics.get('no_ans_em', 0):.2f}"
            )
            ...
            print(
                f"Patience: {patience_counter}/{config.training.vae_patience} (Best Val Loss: {best_vae_loss:.4f})"
            )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(
            f"    HasAns: F1 = {val_metrics['has_ans_f1']:.2f}, EM = {val_metrics['has_ans_em']:.2f} (Count: {val_metrics['total_has_ans']})"
        )
        print(
            f"    NoAns:  F1 = {val_metrics['no_ans_f1']:.2f}, EM = {val_metrics['no_ans_em']:.2f} (Count: {val_metrics['total_no_ans']})"
        )
        print(
            f"    Preds:  HasAns = {val_metrics['pred_has_ans']}, NoAns = {val_metrics['pred_no_ans']}"
        )
