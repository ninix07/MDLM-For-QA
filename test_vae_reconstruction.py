...
def test_vae_reconstruction():
    ...
    # Setup - use config to ensure architecture matches training
    cfg = get_config()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    tokenizer = AutoTokenizer.from_pretrained(cfg.model.base_encoder)

    # Add null token
    null_token = cfg.model.null_ans_token
    if null_token not in tokenizer.get_vocab():
        tokenizer.add_special_tokens({"additional_special_tokens": [null_token]})

    # Create VAE with CORRECT architecture matching LatentDiffusionQA
    # BUG #8 FIX: Use config values instead of hardcoded wrong values
    # BUG #9 FIX: Use config.model.latent_seq_len (now 16 instead of 8)
    vae = SequenceVAE(
        vocab_size=len(tokenizer),
        embedding_dim=768,  # BERT base hidden size
        latent_dim=cfg.model.vae_latent_dim,  # 256 from config (was wrongly 128)
        num_layers=6,  # Match LatentDiffusionQA (was wrongly 4)
        num_heads=12,  # Match LatentDiffusionQA (was wrongly 8)
        dropout=cfg.model.dropout,
        pad_token_id=tokenizer.pad_token_id,
        latent_seq_len=cfg.model.latent_seq_len,  # 16 from config (was wrongly 8)
    ).to(device)
    ...
    if os.path.exists(checkpoint_path):
        ...
        if isinstance(checkpoint, dict):
            if "vae_state_dict" in checkpoint:
                ...
                vae_state_dict = {
                    k.replace("vae.", ""): v for k, v in checkpoint.items() if k.startswith("vae.")
                }
        else:
            vae_state_dict = {
                k.replace("vae.", ""): v
    ...
    print(
        f"Searching for {num_answerable} answerable and {num_unanswerable} unanswerable samples in first {max_search} examples..."
    )
    ...
    for i in range(min(10, len(dataset.examples))):
        ...
        print(
            f"  Example {i}: is_impossible={example.is_impossible}, answer='{example.answer[:50]}...'"
        )
    ...
    print(
        f"Perfect reconstructions (F1=1.0): {perfect_reconstructions}/{len(results)} ({perfect_reconstructions/len(results):.1%})"
    )
    print(
        f"Good reconstructions (F1≥0.8): {good_reconstructions}/{len(results)} ({good_reconstructions/len(results):.1%})"
    )
    print(
        f"Fair reconstructions (F1≥0.5): {fair_reconstructions}/{len(results)} ({fair_reconstructions/len(results):.1%})"
    )
